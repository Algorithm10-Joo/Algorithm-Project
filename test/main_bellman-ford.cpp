...
int main() {
   ...
   // 화재 발생 초기화
    ...
    // edmonds-karp 알고리즘으로 경로 찾기
    std::vector<std::string> path = bellmanFord(nodes, playerNodeCode, exitNodeCode, fireNodes);
    ...
    while (window.isOpen()) {
        ...
        if (fireClock.getElapsedTime().asSeconds() > 0.8 * maxTravelTime) {
            ...
            // 경로 재계산 및 색상 변경
            std::vector<std::string> newPath = bellmanFord(nodes, path[currentPathIndex], exitNodeCode, fireNodes);
